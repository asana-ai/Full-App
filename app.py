...
def load_pose_model(pose_name):
    ...
    safe_pose_name = pose_name.lower().replace(' ', '_').replace("'", '')
    model_filename = f"{safe_pose_name}_classifier.pkl"
...
def update_poses_json_with_model_paths():
    ...
    try:
        ...
        for pose in data['poses']:
            pose_name = pose['name']
            safe_pose_name1 = pose_name.lower().replace(' ', '_').replace("'", '')
            model_filename = f"{safe_pose_name1}_classifier.pkl"
            model_path = os.path.join('models', model_filename)
